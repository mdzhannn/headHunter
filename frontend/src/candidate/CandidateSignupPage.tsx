...
export default function CandidateSignupPage() {
  ...
  const sendOtp = async () => {
    if (!fullName.trim()) return notify('Введите ФИО', 'error');
    if (!email.trim()) return notify('Введите email', 'error');
    setLoading(true);
    try {
      await candidateAuth.registerSendEmailOtp(fullName.trim(), email.trim());
      ...
      if (status === 409) notify('Этот email уже зарегистрирован', 'error');
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await candidateAuth.registerVerifyEmailOtp(email.trim(), otpCode);
      setStep('password');
      notify('Email подтвержден. Теперь задайте пароль');
    } catch {
  ...
  const completeRegistration = async () => {
    if (!canSubmitPassword) return notify('Пароль должен быть не ниже среднего', 'error');
    setLoading(true);
    try {
      await candidateAuth.completeRegistration(email.trim(), password);
      const role = await login(email.trim(), password);
      if (role === 'ADMIN') nav('/admin/dashboard', { replace: true });
      else if (role === 'EMPLOYER') nav('/employer', { replace: true });
      else nav('/app/register', { replace: true });
    } catch {
      notify('Не удалось завершить регистрацию', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="w-full max-w-md p-8 shadow-md border-slate-200">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            {step === 'form' && 'Введите ФИО и email'}
            {step === 'otp' && 'Подтвердите email кодом из письма'}
            {step === 'password' && 'Придумайте пароль для входа'}
          </p>
        </div>
